...
def train_patched_infection_model(infection_trainer, train_dataloader, val_dataloader):
    epoch = 0
    losses = dict(train=[],val=[])
    min_loss = np.Inf
    if FROM_SAVE['inf']:
...
def main():
    ...
    summary(infection_trainer.model,(1,IN_SIZE//PATCH_FACTOR,IN_SIZE//PATCH_FACTOR))

    # Train Infection model
    # train_infection_model(lung_trainer, infection_trainer, train_dataloader, val_dataloader)
    train_patched_infection_model(infection_trainer, train_dataloader, val_dataloader)
